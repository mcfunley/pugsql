...
def test_module():
    assert pugsql.module("tests/sql").sqlpaths == {
        "tests/sql",
    }
...
class PugsqlTest(TestCase):
    ...
    def test_update(self):
        self.assertEqual(
            1, self.fixtures.update_username(user_id=3, username="dottie")
        )
    ...
    def test_positional_args_mistake(self):
        with pytest.raises(
            ...
            self.fixtures.find_by_username_or_id(1, ("oscar", "dottie"))
    ...
    def test_nesting_transactions_rollback_inner(self):
        with self.fixtures.transaction() as outer:
            ...
            users = {
                u["username"]
                for u in self.fixtures.find_by_usernames(
